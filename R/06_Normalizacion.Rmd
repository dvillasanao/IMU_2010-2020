...
$\circ \: {DP}_{2}^{i}$: es el valor del índice de marginación de la AGEB $i$,   
   ...
   tab_options(heading.title.font.size = 12, 
    ...
    tab_style(style = list(cell_text(align = "center",
     ...
     cols_label(`desvest` = md("**Desviación estándar**"), 
                ...
                `desvest.inversa` = md("**Desviación estándar m. inversa**")) %>%
...
DP2_2020 %>% 
 #rename("AÑO" = "ANIO") %>%
  slice(1:20) %>%
  gt() %>%
   tab_header(title = "Índice de marginación a nivel AGEB, 2020") %>%
    tab_options(heading.title.font.size = 12, 
                heading.align = "center",
                heading.subtitle.font.size = 10,
                table.align = "center",
     ...
     tab_style(style = list(cell_text(align = "center",
                                       weight = 'bold')),
                locations = list(cells_title(groups = c("title")))) %>%
      fmt_integer(columns = "POB_TOTAL", sep_mark = " ") %>%
       fmt_number(columns = c(11:21), decimals = 1) %>%
        fmt_number(columns = c(22, 24), decimals = 3) %>%
